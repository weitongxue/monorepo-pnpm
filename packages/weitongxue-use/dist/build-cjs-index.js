 ...
 * @internal
 */
/*#__PURE__*/ PolySymbol((process.env.NODE_ENV !== 'production') ? 'route location' : 'rl');
/**
...
var defaultResponseConfig = {
    tableDataKey: 'data.list',
    totalKey: 'data.total'
};
function useList(config) {
    var _a = config.request.params, requestParams = _a === void 0 ? {} : _a;
    var cacheConfig = cloneDeep(config);
    cacheConfig.response = Object.assign({}, defaultResponseConfig, cacheConfig.response || {});
    var isUpdateRouterUrl = cacheConfig.isUpdateRouterUrl;
    var _b = cacheConfig.request, _c = _b.pageNumKey, pageNumKey = _c === void 0 ? 'pageNum' : _c, _d = _b.pageSizeKey, pageSizeKey = _d === void 0 ? 'pageSize' : _d, handleParams = _b.handleParams, handleValidate = _b.handleValidate, handleCustomReset = _b.handleCustomReset, resetApi = _b.resetApi;
    var api = cacheConfig.request.api;
    var _e = cacheConfig.response, tableDataKey = _e.tableDataKey, totalKey = _e.totalKey, handleResponseData = _e.handleResponseData;
    var updateRouteUrl = function (query) {
    ...
    var params = ref(cloneDeep(requestParams));
    var resData = ref({});
    /**
     * 列表查询
     */
    var handleSearch = function (pageNum) {
        if (pageNum === void 0) { pageNum = 1; }
        if (pageNumKey in params.value) {
            params.value[pageNumKey] = pageNum;
        }
        var apiParams = Object.assign({}, params.value);
        if (handleValidate && !handleValidate(params.value)) {
            return null;
        }
        if (handleParams) {
            apiParams = handleParams(cloneDeep(params.value));
        }
        if (resetApi) {
            api = resetApi();
        }
    ...
    var handleReset = function () {
        if (handleCustomReset) {
            params.value = handleCustomReset(params.value, cloneDeep(requestParams));
        }
        else {
     ...
     * 切换页码 刷新列表
     */
    var handleCurrentChange = function (pageNum) { return handleSearch(pageNum); };
    return {
        params: params,
        resData: resData,
        tableData: tableData,
