...
function clear() {
  ...
  const oldTarget = process.env.NODE_ENV !== 'production' ? isMap$2(target) ? new Map(target) : new Set(target) : undefined; // forward the operation before queueing reactions
...
function cloneVNode(vnode, extraProps, mergeRef = false) {
  ...
  const cloned = {
    ...
    /* HOISTED */
    && isArray$2(children) ? children.map(deepCloneVNode) : children,
    target: vnode.target,
...
function useList(config) {
    ...
    const { pageNumKey = 'pageNum', pageSizeKey = 'pageSize', handleParams, handleValidate, handleCustomReset, resetApi, } = cacheConfig.request;
    let { api } = cacheConfig.request;
    const { tableDataKey, totalKey, handleResponseData } = cacheConfig.response;
    const updateRouteUrl = (query = {}) => {
    ...
    const handleSearch = (pageNum = 1) => {
        ...
        let apiParams = Object.assign({}, params.value);
     ...
     * 切换页码 刷新列表
     */
    const handleCurrentChange = (pageNum) => handleSearch(pageNum);
    return {
        params,
        resData,
        tableData,
